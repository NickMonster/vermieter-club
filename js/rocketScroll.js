...
RS.RocketScroll.prototype.bindEvents = function () {
  ...
  this.el.onmousemove = function (e) {
    ...
    e = e || window.event; // IE Fix
    self.scrollDiv.scrollTop =
      (e.clientY - self.clientY) / self.ratio + self.scrollTop;
  };
  ...
  this.scrollbar.onclick = function (e) {
    ...
    self.scrollDiv.scrollTop =
      (layerY / self.totalHandle) * self.totalScrollable;
  };
...
RS.RocketScroll.prototype.setMouseUpAndEnableSelection = function (instance) {
  instance.contentDiv.className = instance.contentDiv.className.replace(
    instance.UNSELECTABLE_CLASS,
    '',
  );
...
RS.RocketScroll.prototype.refresh = function (updateImagesOnload) {
  ...
  this.handle.style.height =
    this.scrollDiv.clientHeight *
      (this.scrollDiv.clientHeight / this.contentDiv.clientHeight) +
    'px';

  // Refreshing scroll bar position and ratio
  // Should be called on content change
  this.totalScrollable =
    this.contentDiv.clientHeight - this.scrollDiv.clientHeight;
